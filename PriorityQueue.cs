...
namespace 中国象棋
{
	//https://github.com/dotnet/reactive/blob/master/Rx.NET/Source/src/System.Reactive/Internal/PriorityQueue.cs
	...
	internal sealed class PriorityQueue<T>
	{
		...
		public void Enqueue(T item)
		{
			...
			var index = _size++;
			_items[index] = new IndexedItem { Value = item, Id = ++_count, comparison = comparison };
			Percolate(index);
		}
		...
		private struct IndexedItem : IComparable<IndexedItem>
		{
			...
			{
				var c = comparison(Value, other.Value);
				if (c == 0)
				{
